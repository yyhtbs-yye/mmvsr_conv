...
from mmvsr.registry import TRANSFORMS
...
class GenerateFrameIndices(BaseTransform):
    ...
    def __init__(self, interval_list, frames_per_clip=99, seed=None):

        self.interval_list = interval_list
        self.frames_per_clip = frames_per_clip
        if seed is not None:
            self.random_state = np.random.RandomState(seed)  # Using a local RandomState instead of global
    ...
    def transform(self, results):
        ...
        if hasattr(self, 'random_state'):
            interval = self.random_state.choice(self.interval_list)
        else: 
        ...
        while (start_frame_idx < 0) or (end_frame_idx > frames_per_clip):
            if hasattr(self, 'random_state'):
                center_frame_idx = self.random_state.randint(0, frames_per_clip + 1)
            else: 
    ...
    def __repr__(self):
        ...
        repr_str += (f'(interval_list={self.interval_list}, frames_per_clip={self.frames_per_clip})')
        if hasattr(self, 'random_state'):
            repr_str += f', seed={self.random_state.seed}'
