...
class TSAFusion(BaseModule):
    ...
    def forward(self, aligned_feat):
        ...
        # Apply temporal_attn2 to the entire aligned feature set, 
        emb = self.temporal_attn2(aligned_feat.view(-1, c, h, w))
        # then reshape for further processing.
        emb = emb.view(n, t, -1, h, w)  # (n, t, c, h, w)

        corr_l = []  # correlation list
        # Compute correlation for each frame with the reference
        for i in range(t):
            emb_neighbor = emb[:, i, :, :, :]
            corr = torch.sum(emb_neighbor * embedding_ref, 1)  # (n, h, w)
            corr_l.append(corr.unsqueeze(1))  # (n, 1, h, w)
        # aggregate them, and apply sigmoid to get attention weights.
        ...
        # Attention: Multiply the original aligned features by these attention weights (corr_prob) for temporal fusion.
        aligned_feat = aligned_feat.view(n, -1, h, w) * corr_prob

        # fusion
        # Pass the temporally fused features through a fusion layer to combine information across all frames.
        # [B, T*C, H, W] -> [B, C, H, W] , T is aggregated
        feat = self.feat_fusion(aligned_feat)

        # spatial attention
        # Apply spatial attention layers to the fused feature, 
        # including different levels of pooling and upscaling.
        ...
        # The attention features -> maximum pooling (max_pool) and average pooling (avg_pool).
        attn_max = self.max_pool(attn_level)
        attn_avg = self.avg_pool(attn_level)
        # Attention Feature Fusion: The pooled features are concatenated -> spatial_attn_l2.
        ...
        # Leverage sigmoid activation
        attn = torch.sigmoid(attn)
        # and a specific multiplication factor to ensure the attention values effectively scale the features.
        feat = feat * attn * 2 + attn_add # after initialization, * 2 makes (attn * 2) to be close to 1.
        return feat
