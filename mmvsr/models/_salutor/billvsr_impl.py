...
from mmvsr.models._temporal_propagator.feedforward import FirstOrderUnidirectionalSlidingWindowPropagator as Propagator
...
class BillVSRImpl(BaseModule):
    ...
    def forward(self, lrs):
        ...
        forward_flows, _ = self.compute_flow(lrs)

        feats_ = self.preproc(lrs)

        feats_l1 = self.propagator_l1(feats_, forward_flows, [])
        feats_l2 = self.propagator_l2(feats_l1, forward_flows, [feats_])
        feats_l3 = self.propagator_l3(feats_l2, forward_flows, [feats_, feats_l1])
        feats_l4 = self.propagator_l4(feats_l3, forward_flows, [feats_, feats_l1, feats_l2])
