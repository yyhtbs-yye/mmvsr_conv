...
from mmvsr.apis.inferencers.inpainting_inferencer import InpaintingInferencer
...
class InpaintingGradio:
    ...
    def get_package_path() -> str:
        p = subprocess.Popen(
            'pip show mmvsr', shell=True, stdout=subprocess.PIPE)
        out, err = p.communicate()
        out = out.decode()
        if 'Location' not in out:
            InpaintingGradio.send_notification('module mmvsr not found',
                                               InpaintingGradio.error_color,
                                               'error')
            raise Exception('module mmvsr not found')
