...
There are some mmvsr demos in this folder. We provide python command line usage here to run these demos and more guidance could also be found in the [documentation](https://mmvsr.readthedocs.io/en/latest/user_guides/inference.html)

Table of contents:

[1. Download sample images or videos](#1-download-sample-images-or-videos)

[2. mmvsr inference demo](#2-mmvsr-inference-demo)
...
We prepared some images and videos for you to run demo with. After mmvsr is well installed, you could use demos in this folder to infer these data.
...
```shell
python demo/mmvsr_inference_demo.py --print-supported-models
```

print all supported tasks for inference.

```shell
python demo/mmvsr_inference_demo.py --print-supported-tasks
```

print all supported models for one task, take 'Image2Image' for example.

```shell
python demo/mmvsr_inference_demo.py --print-task-supported-models 'Text2Image'
```

### 2.2 Perform inference with command line

You can use the following commands to perform inference with a mmvsr model.

Usage of python API can also be found in this [tutotial](./mmvsr_inference_tutorial.ipynb).

```shell
python demo/mmvsr_inference_demo.py \
    [--img] \
    [--video] \
    [--label] \
    [--trimap] \
...
First, put your checkpoint path in `./checkpoints`, *e.g.* `./checkpoints/stylegan2_lions_512_pytorch_mmvsr.pth`. For example,

```shell
mkdir checkpoints
cd checkpoints
wget -O stylegan2_lions_512_pytorch_mmvsr.pth https://download.openxlab.org.cn/models/qsun1/DragGAN-StyleGAN2-checkpoint/weight//StyleGAN2-Lions-internet
```
